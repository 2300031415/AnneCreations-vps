    ...
    (set, get) => ({
      ...
      startCheckout: async () => {
        ...
        try {
          ...
          const message = getApiErrorMessage(err, 'Checkout start failed');
      ...
      autoApplyCoupon: async (orderId) => {
        ...
        try {
          ...
          set({ applyCouponError: getApiErrorMessage(err, 'Auto coupon not available'), loading: false });
          return null;
        }
      },
      ...
      fetchCheckoutStatus: async (id) => {
        ...
        try {
          ...
          set({
            error: getApiErrorMessage(err, 'Failed to fetch checkout status'),
            loading: false,
          });
          throw new Error(getApiErrorMessage(err, 'Failed to fetch checkout status'));
        }
      },
      ...
      cancelCheckout: async (id) => {
        ...
        try {
          ...
          const message = getApiErrorMessage(err, 'Failed to cancel checkout');
          set({ error: message, loading: false });
          throw new Error(message);
        }
      },
      ...
      paymentCreate: async (orderId) => {
        try {
          ...
          set({
            loading: false,
            error: getApiErrorMessage(err, 'Payment initialization failed'),
          });
          throw new Error(getApiErrorMessage(err, 'Payment initialization failed'));
        }
      },
