...
export const useDownloadStore = create((set) => ({
  ...
  fetchDownloads: async (page = 1) => {
    ...
    try {
      const response = await axiosClient.get(`/api/orders/my?status=paid&page=${page}`);
  ...
  searchDownloads: async (searchQuery) => {
    ...
    try {
      const response = await axiosClient.get(
        `/api/orders/my?status=paid&search=${encodeURIComponent(searchQuery)}&page=1&limit=100`
      );
      const { data, pagination } = response.data;
