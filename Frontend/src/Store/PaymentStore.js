    ...
    (set) => ({
      ...
      createPaymentInfo: async (orderId) => {
        ...
        try {
          ...
          throw new Error(getApiErrorMessage(err, 'Unable to initialize payment'));
        } finally {
      ...
      verifyPayment: async (payload) => {
        ...
        try {
          ...
          set({ verificationResult: res.data?.data || null });
          return res.data?.data || null;
        } catch (err) {
          console.error('Payment verification failed:', err);
          throw new Error(getApiErrorMessage(err, 'Payment verification failed'));
        } finally {
          set({ paymentLoader: false });
        }
      },
