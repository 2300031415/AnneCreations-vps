...
const Banner = () => {
          ...
          if (Array.isArray(bannerList)) {
            bannerList.forEach(banner => {
              if (banner.status !== false && banner.images && Array.isArray(banner.images)) {
                banner.images.forEach((imgEntry, idx) => {
                  const imagePath = typeof imgEntry === 'string' ? imgEntry : imgEntry?.image;
                  const isActive = typeof imgEntry === 'string' ? true : imgEntry?.status !== false;
                  if (!imagePath || !isActive) {
                    return;
                  }
                  extractedImages.push({
                    id: `${banner._id}-${idx}`,
                    image: `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/${imagePath}`,
  ...
  if (loading) {
    return (
      <div className="w-full flex items-center justify-center h-[600px] bg-(--primary)">
        <p className="text-(--secondary) text-lg">{t('common.loading_banners', 'Loading banners...')}</p>
      </div>
    );
  }
