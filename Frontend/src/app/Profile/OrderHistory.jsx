...
const OrderHistory = () => {
  ...
  return (
    <div className="rounded-xl border-2 border-(--primary) my-2 lg:my-0 lg:mx-8 w-full">
      ...
      <div className="flex flex-col gap-4 p-4">
        ...
        {!isLoading &&
          orders.map((order) => (
            <div
              key={order._id}
              className="bg-white shadow-lg rounded-xl p-4 grid grid-cols-1 sm:grid-cols-[1fr_auto] items-center gap-4 w-full"
            >
              {/* Single Row Data */}
              <div className="flex-1 flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 w-full sm:w-auto">
                ...
                <span className="flex-1 min-w-[80px]">
                  <span className="font-semibold text-(--secondary)">Order Date: </span>
                  {order.createdAt ? new Date(order.createdAt).toLocaleDateString() : 'N/A'}
                </span>

                <span className="flex-1 min-w-[80px]">
                  <span className="font-semibold text-(--secondary)">No of Products: </span>
                ...
                <span className="flex-1 min-w-[100px] font-bold text-(--secondary)">
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="bg-(--primary) border-2 border-(--primary) hover:bg-white text-(--secondary) hover:text-(--primary) font-semibold px-6 py-2 rounded-lg transition cursor-pointer"
