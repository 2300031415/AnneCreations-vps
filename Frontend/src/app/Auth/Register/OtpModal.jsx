...
const OtpModal = ({ open, onClose, target, formData }) => {
  ...
  const { sendOtp, verifyOtpAndRegister } = useAuthStore();
  ...
  const handleVerifyAndRegister = async () => {
    ...
    try {
      const result = await verifyOtpAndRegister(otp, formData);
        ...
        <Box display="flex" justifyContent="center" mt={3} gap={2}>
          <Button
            ...
            sx={{
              ...
              fontWeight: 700,
              textTransform: "none",
              "&:hover": { backgroundColor: "white", color: "var(--secondary)", border: "1px solid var(--primary)" },
            }}
          >
