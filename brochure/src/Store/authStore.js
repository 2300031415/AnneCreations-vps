...
export const useAuthStore = create(
  persist(
    (set, get) => ({
      ...
      setLoginPopupOpen: (open) => set({ loginPopupOpen: open }),
      ...
      login: async (mobile, password) => {
        ...
        try {
          const { data } = await axios.post(`${API_URL}/api/customers/login`, { mobile, password }, { withCredentials: true });
          set({
            user: data.customer,
      ...
      sendOtp: async (mobile, clientSource = 'web') => {
        ...
        try {
          ...
          // Fix: Pass payload.timestamp explicitly to ensure signature matches payload
          const signature = generateOTPSignature(payload, SECRET_KEY, payload.timestamp);
