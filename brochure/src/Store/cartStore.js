...
const useCartStore = create(
  persist(
    (set, get) => ({
      ...
      setPendingQuickAdd: (product) => set({ pendingQuickAdd: product }),
      ...
      // ✅ Update item in cart
      updateCartItem: async (productId, options) => {
        set({ cartLoader: true });
        try {
          // Extract IDs from options correctly
          const optionIds = options.map((opt) => {
            if (typeof opt === "string") return opt;
            if (opt._id) return opt._id;
            if (opt.option && opt.option._id) return opt.option._id;
            return opt;
          });

          await axiosClient.put(`/api/cart/items/${productId}`, {
            options: optionIds,
          });
          await get().getCartItem();
        } catch (error) {
