...
export interface ProductFormData {
  ...
  backStitches?: string | number;
  handStitches?: string | number;
  frontStitches?: string | number;
  overallStitches?: string | number;
  createdAt?: string;
  todayDeal?: boolean;
  todayDealExpiry?: string;
}
...
export function ProductDialog({ children, mode = "create", product }: ProductDialogProps) {
  ...
  useEffect(() => {
    if (mode === "edit" && fetchedProduct) {
      ...
      setFormData(prev => ({
        ...
        backStitches: fetchedProduct.backStitches || "",
        handStitches: fetchedProduct.handStitches || "",
        frontStitches: fetchedProduct.frontStitches || "",
        overallStitches: fetchedProduct.overallStitches || "",
        createdAt: fetchedProduct.createdAt ? new Date(fetchedProduct.createdAt).toISOString().split('T')[0] : "",
        todayDeal: !!fetchedProduct.todayDeal,
        todayDealExpiry: fetchedProduct.todayDealExpiry ? new Date(fetchedProduct.todayDealExpiry).toISOString().split('T')[0] : "",
      }));
    }
  }, [fetchedProduct, mode]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.backStitches) form.append("backStitches", String(formData.backStitches));
      if (formData.handStitches) form.append("handStitches", String(formData.handStitches));
      if (formData.frontStitches) form.append("frontStitches", String(formData.frontStitches));
      if (formData.overallStitches) form.append("overallStitches", String(formData.overallStitches));
      if (formData.createdAt) form.append("createdAt", new Date(formData.createdAt).toISOString());
      form.append("todayDeal", formData.todayDeal ? "true" : "false");
      if (formData.todayDealExpiry) form.append("todayDealExpiry", new Date(formData.todayDealExpiry).toISOString());
      ...
      <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 py-4">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button type="submit" className="bg-[#ccd88f] text-[#311807]" disabled={isLoading}>
              {isLoading ? (
                <>
