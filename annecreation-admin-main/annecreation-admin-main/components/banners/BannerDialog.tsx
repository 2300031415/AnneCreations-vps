...
export function BannerDialog({ children, mode = "create", banner }: BannerDialogProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (formData.deviceType === "mobile" && !formData.mobileFiles?.length && !formData.mobileImages?.length) {
        throw new Error("At least one mobile image is required")
      }

      if (formData.deviceType === "web" && !formData.websiteFiles?.length && !formData.websiteImages?.length) {
        throw new Error("At least one website image is required")
      }
      ...
      <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 py-4">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button type="submit" className="bg-[#ccd88f] text-[#311807] hover:bg-[#ccd88f]/90" disabled={loading}>
