...
export function OrdersList() {
  ...
  const [allAvailableStatuses, setAllAvailableStatuses] = useState<string[]>(['Pending', 'paid', 'cancelled', 'Shipped', 'Processing']);
  ...
  // Sync state with URL status parameter or filter
  useEffect(() => {
    if (filterParam === 'new') {
      // "New Orders" logic: Last 30 days, all statuses
      const end = new Date();
      ...
      // Clear date filters when specific status is requested via URL
      setDateFrom("");
      setDateTo("");
      setAppliedDateFrom("");
      setAppliedDateTo("");
    } else {
      // If no status in URL (e.g. "All Orders"), reset to empty to allow "Show All" logic to take over
      setSelectedStatuses([]);
      // Clear date filters unless manually applied? 
      // For "All Orders" link, we usually want to reset everything.
      // But if user is just navigating, maybe we shouldn't?
      // For now, if no params, we assume fresh "All Orders" view.
      if (!filterParam) {
        setDateFrom("");
        setDateTo("");
        setAppliedDateFrom("");
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <CardTitle className="text-xl font-bold text-gray-900">
              {filterParam === 'new' ? "New Orders" : statusFromUrl ? `${statusFromUrl} Orders` : "All Orders"}
            </CardTitle>
