...
import { useGetYearlyNewCustomersQuery } from "@/lib/redux/api/dashboardApi"
...
interface CustomerGrowthChartProps extends React.HTMLAttributes<HTMLDivElement> { }

export function CustomerGrowthChart({ className, ...props }: CustomerGrowthChartProps) {
    const currentYear = new Date().getFullYear();
    const [year, setYear] = useState(currentYear);
    const { data, isLoading } = useGetYearlyNewCustomersQuery({ year });
    ...
    return (
        <Card className={`glass-card border-none ring-1 ring-[#311807]/5 ${className}`} {...props}>
            <CardHeader className="flex flex-row items-center justify-between pb-8 bg-transparent">
                <div>
                    <CardTitle className="text-lg font-bold text-[#311807] font-poppins">Customer Growth</CardTitle>
                    <CardDescription className="text-[#311807]/50 font-inter">New registrations per month</CardDescription>
                </div>
                ...
                {isLoading ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                                            ...
                                            <div className="rounded-xl border border-[#311807]/10 bg-white/90 backdrop-blur-md p-3 shadow-xl text-xs">
                                                <div className="grid grid-cols-2 gap-4">
                                                    ...
                                                    <div className="flex flex-col border-l border-[#311807]/10 pl-4">
                                                        <span className="text-[0.65rem] uppercase font-bold text-[#311807]/40">New Users</span>
                            ...
                            />
                            <Bar dataKey="count" radius={[6, 6, 0, 0]} fill="#311807" barSize={24} opacity={0.8} />
                        </BarChart>
                    </ResponsiveContainer>
                )}
            </CardContent>
